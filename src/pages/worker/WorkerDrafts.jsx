      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-1">Resume your saved drafts before submitting them.</p>
          </div>
          <button
          ...
          >
            <FiFileText className="h-4 w-4" /> View Submitted Reports
          </button>
        </div>
        ...
        {drafts.length === 0 ? (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
            ...
            <p className="text-xl font-semibold text-gray-700">No drafts found</p>
            <p className="text-gray-500 mt-2">Any saved draft will appear here for you to continue later.</p>
          </div>
        ) : (
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 text-sm text-gray-500 uppercase tracking-wide">
                      <FiClock className="h-4 w-4" /> Draft saved {new Date(draft.updatedAt || draft.createdAt).toLocaleDateString()}
                    </div>
                    <h2 className="text-xl font-semibold text-gray-900">{draft.formId?.title || 'Untitled Form'}</h2>
                    <p className="text-sm text-gray-600">Event: {draft.eventId?.name || 'Unknown event'}</p>
                    <p className="text-sm text-gray-600">Progress: {draft.completionPercentage ?? 0}% complete</p>
                  </div>
                  ...
                  >
                    Continue Draft <FiArrowRight className="h-4 w-4" />
                  </button>
                </div>
              </div>
            ))}

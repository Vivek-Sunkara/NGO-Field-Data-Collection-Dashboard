...
const IssuesStep = ({ formData, onChange, errors, ui = LEGACY_FORM_UI }) => {
  const english = getLegacyEnglishOptions(ui);
  const { labels, placeholders, helpers } = ui;
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-gray-600 mb-3">{helpers.issuesHint}</p>

        <div className="space-y-2">
          {english.issueOptions.map((issue, index) => (
            <label key={issue} className="flex items-center p-3 border-2 border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 transition">
              ...
              />
              <span className="ml-3 text-gray-700 font-medium">{ui.issueOptions[index] || issue}</span>
            </label>
          ))}
        </div>
      </div>
      ...
      <div>
        ...
        >
          ...
          <p className="text-xs text-gray-600 mt-1">{helpers.imagesHint}</p>
          <input
            ref={fileInputRef}
            type="file"
            multiple
